...
def noise_prediction(window, interval):
    N = int(window / interval)
    d = bw_hdd_record[-N:]
    xf = fft.fftfreq(N, interval)
    yf = fft.fft(d) / N
    amp = np.abs(yf)

    sorted_indices = np.argsort(amp)[::-1]
    s_freq = xf[sorted_indices[:amp_num]]
    s_amp = amp[sorted_indices[:amp_num]]
    s_complex = yf[sorted_indices[:amp_num]]
    s_phase = np.angle(np.array(s_complex))
    
    dc = s_amp[0]
    t = np.arange(0, window, interval)
    sig = dc
    for i in range(1, amp_num):
        sig += s_amp[i] * np.cos(2*np.pi * s_freq[i] * t + s_phase[i])
...
def read_with_collision_detection(app_tag, size, interval, exp_time):
    ...
    for i in range(int(exp_time/interval)):
        ...
        if bw_pre > bw_high_bound_col:
            ...
            aug_factor = bw_pre / bw_high_bound_col # fixed factor
            # aug_factor = 1.0 / random.uniform(1.0, 2*bw_high_bound_col/bw_pre-1) # random factor
        print("Collision Factor: ", aug_factor)
        after_size *= aug_factor
        col_record.append(aug_factor)
        ...
        print("Time = %.2f s, Bw_hdd = %.2f MB/s" % (ana_time, bw_hdd))
        time.sleep(interval)
...
def read_with_combined(app_tag, size, interval, exp_time, predict_result, bw_per_second):
    ...
    for i in range(int(exp_time/interval)):
        ...
        if pred_accuracy > 1 - accuracy_threshold:
            ...
            # aug_factor = bw_pre / bw_high_bound_col # fixed random factor
                aug_factor = min(1, random.expovariate(bw_high_bound_col / (bw_pre+0.01))) # real random factor
                # print("Collision Factor: ", aug_factor)
                aug_ratio = aug_factor
