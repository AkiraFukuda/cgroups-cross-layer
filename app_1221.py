...
def noise_prediction(window, interval):
    ...
    if amp[0] > 1e-10:
        threshold = np.max(np.delete(amp, 0, axis=0))*freq_high_ratio
        dc = amp[0]
        start_index = 1
    else:
        threshold = np.max(amp) * freq_high_ratio
        dc = 0.0
        start_index = 0
    ...
    for i in range(start_index, N):
        if amp[i] >= threshold:
            s_freq.append(freq[i])
            s_amp.append(amp[i])
            s_complex.append(yf[i])

    # Calculate s_phase
    s_phase = np.arctan2(np.array(s_complex).imag, np.array(s_complex).real)
    for i in range(len(s_phase)):
        if np.fabs(s_phase[i]) < 1e-10:
            s_phase[i] = 0.0
...
def read_with_collision_detection_twice(app_tag, size, interval, exp_time):
    ...
    for i in range(int(exp_time/interval)):
        ...
        if bw_pre > bw_high_bound_col:
            ...
            # random_factor = bw_pre / bw_high_bound_col # fixed random factor
            random_factor = 1.0 / random.uniform(1.0, 2*bw_high_bound_col / bw_pre) # real random factor
        print("Collision Factor: ", random_factor)
