...
def read_with_pred_original(app_tag, size, interval, exp_time, predict_result, bw_per_second):
    global bw_record, io_time_record, aug_record, bw_hdd_record
    gstart = time.time()
    for i in range(int(exp_time/interval)):
        tmp = time.time() - gstart
...
def read_with_pred_new(app_tag, size, interval, exp_time, predict_result, bw_per_second):
    global bw_record, io_time_record, aug_record, bw_hdd_record
    phase_chg = 0
    pred_error = 0
    sum_bw_hdd = 0
...
def work(app_tag, read_size, interval, exp_time, ratio):
    ...
    while True:
        if time.time() - st_time >= exp_time*ratio:
            read_with_pred_original(app_tag, read_size, interval,
                                exp_time, bw_predicted, bw_per_second)
            break
    while True:
        if time.time() - st_time >= exp_time*ratio*2:
            read_with_pred_new(app_tag, read_size, interval,
                                exp_time, bw_predicted, bw_per_second)
            break
